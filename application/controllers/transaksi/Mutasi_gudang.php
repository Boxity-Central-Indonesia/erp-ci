...
class mutasi_gudang extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        checkAccess($this->session->userdata('fiturview')[16]);
    }
    ...
    public function index()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $data['menu'] = 'mutasigdg';
            $data['title'] = 'Transaksi Mutasi Gudang';
            $data['view'] = 'transaksi/v_mutasi_gudang';
            $data['scripts'] = 'transaksi/s_mutasi_gudang';
            ...
            $FiturID = 16; //FiturID di tabel serverfitur
            $canEdit = 0;
            $edit = [];
            ...
            foreach ($records as $record) {
                ...
                if ($canEdit == 1 && $canDelete == 1) {
                    $temp['btn_aksi'] = '<a class="btnfitur" href="' . base_url('transaksi/mutasi_gudang/detail/' . base64_encode($temp['NoTrans'])) . '" type="button" title="Detail Transaksi"><span class="fa fa-list" aria-hidden="true"></span></a>' .
    ...
    public function simpan()
    {
        ...
        if (!($this->input->post('NoTrans') != null && $this->input->post('NoTrans') != '')) {
            ...
            $insertdata['JenisTransaksi'] = 'MUTASI';
        ...
        if ($res) {
            ...
            $this->logsrv->insert_log([
                'Action' => $aksi,
                'JenisTransaksi' => 'Transaksi Mutasi Gudang',
                'Description' => $ket . ' data transaksi mutasi gudang ' . $id
            ]);
            echo json_encode([
                'status' => true,
        ...
        if ($countitem > 0) {
            ...
            if ($res) {
                ...
                $this->logsrv->insert_log([
                    'Action' => 'hapus',
                    'JenisTransaksi' => 'Transaksi Mutasi Gudang',
                    'Description' => 'hapus data transaksi mutasi gudang ' . $kode
                ]);
                echo json_encode([
                    'status' => true,
    ...
    public function detail()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            $data['breadcrumb'][] = array('Name' => '', 'Url' => '#', 'IsAktif' => 0);
            $data['menu'] = 'mutasigdg';
            $data['title'] = 'Detail Transaksi Mutasi Gudang';
            $data['view'] = 'transaksi/v_mutasi_gudang_detail';
            $data['scripts'] = 'transaksi/s_mutasi_gudang_detail';
            ...
            ## select -> fill with all column you need
            $configData['selected_column'] = [
                'i.NoUrut', 'i.NoTrans', 'i.Qty', 'i.HargaSatuan', 'i.Total', 'i.Deskripsi', 'i.JenisStok', 'i.GudangAsal', 'i.GudangTujuan', 'i.SatuanBarang', 'i.IsHapus', 'i.KodeBarang', 'br.NamaBarang'
            ];
            ## display column -> Represent column in view
            // index must same with table column
            // set false if column not in db table (column number, action, etc)
    ...
    {
        $notrans        = escape(base64_decode($this->uri->segment(4)));
        $data['src_url'] = base_url('transaksi/mutasi_gudang/detail/') . $this->uri->segment(4);
        ...
        $sql = [
            'select' => 'i.NoUrut, i.NoTrans, i.Qty, i.HargaSatuan, i.Total, i.Deskripsi, i.JenisStok, i.GudangAsal, i.GudangTujuan, i.SatuanBarang, i.IsHapus, i.KodeBarang, br.NamaBarang',
        ...
        $this->load->view('transaksi/v_mutasi_gudang_detail_cetak', $data);
    }
    ...
    public function checkStock()
    {
        ...
        if ($StokTujuan > ($StokAsal + $QtyLama)) {
            setresponse(HTTP_OK, ['status' => false, 'msg' => 'Jumlah stok tujuan tidak boleh melebihi jumlah stok asal']);
        } else {
