<script>
    ...
    const $table = $('#table-returdetail').DataTable({
        ...
        "ajax": {
            "url": "<?= base_url('transaksi/retur_pembelian/detail'); ?>",
            "type": "GET",
            "data": function(d) {
    ...
    $(document).ready(function() {
        $("#form-simpan").submit(function(e) {
            ...
            let data = {
                JmlDatang: $('#JmlDatang').val(),
                JmlRetur: $('#JmlRetur').val()
            }
            get_response("<?= base_url('transaksi/retur_pembelian/checkitembeli') ?>", data, function(response) {
    ...
    function hapus(kode, kode2) {
        ...
        get_response("<?= base_url('transaksi/retur_pembelian/hapusdetail') ?>", data, function(response) {
            if (response.status === false) {
    ...
    function simpan(self, data_post) {
        post_response("<?= base_url('transaksi/retur_pembelian/simpandetail') ?>", data_post, function(response) {
            if (response.status) {
                $('#ModalTambah').modal('hide');
    ...
    function simpanreturs(self, data_post) {
        post_response("<?= base_url('transaksi/retur_pembelian/simpanretur') ?>", data_post, function(response) {
            if (response.status) {
                $('#ModalRetur').modal('hide');
                showSwal("success", "Informasi", "Berhasil input data").then(function() {
                    self[0].reset();
                    if (response.jenisrealisasi == 'KEMBALI UANG' && response.stj == 'off') {
                        window.location.href = "<?= base_url('transaksi/transaksi_kas/jurnalmanual/') ?>" + btoa("trans_beli") + "/" + btoa(unescape(encodeURIComponent(response.idjurnal))) + "/" + btoa("<?= $IDTransRetur ?>") + "/" + btoa("retur_pembelian/detail");
                    } else {
                        window.location.reload();
                    }
                });
    ...
    function batalretur(kode) {
        ...
        get_response("<?= base_url('transaksi/retur_pembelian/batal') ?>", data, function(response) {
            if (response.status === false) {
    ...
    $("#btntambah").on("click", function() {
        ...
        $('#KodeBarang').change(function () {
            ...
            var idtransbeli = "<?= $dtinduk['IDTransBeli'] ?>";
            // console.log(kodeBrg, idtransbeli);
            $.ajax({
                url: "<?php echo site_url('transaksi/retur_pembelian/getitembeli'); ?>",
                method: "GET",
                data: {KodeBarang: kodeBrg, IDTransBeli: idtransbeli},
                dataType: 'json',
                success: function (data) {
                    if (kodeBrg) {
                        ...
                        $('#JmlDatang').val(data.BarangDatang);
                    } else {
                        $('#SatuanBarang').val('');
                        $('#AdditionalName').val('');
                        $('#HargaJual').val('');
