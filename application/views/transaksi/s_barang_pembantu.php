<script>
    const $table = $('#table-barangpembantu').DataTable({
        "paging": true,
        "lengthChange": false,
        "searching": false,
        "ordering": true,
        ...
        "ajax": {
            "url": "<?= base_url('transaksi/barang_pembantu'); ?>",
            "type": "GET",
            "data": function(d) {
    ...
    $("#combo-gudang").on('change', function() {
        $table.ajax.reload();
        var url = '<?= base_url('transaksi/barang_pembantu/cetak/') ?>' + btoa($("#combo-gudang").val());
