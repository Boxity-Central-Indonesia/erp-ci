<script>
    ...
    const $table = $('#table-kirimbayar').DataTable({
        "paging": false,
        "lengthChange": false,
        "searching": false,
        "ordering": true,
        ...
        "ajax": {
            "url": "<?= base_url('transaksi/transaksi_pembelian/kirimpembayaran'); ?>",
            "type": "GET",
            "data": function(d) {
    ...
    $('#table-kirimbayar tbody').on('click', 'td.details-control i.fa', function () {
        ...
        if (row.child.isShown()) {
            ...
            row.child('<table cellpadding="5" cellspacing="0" border="0" style="background-color:#e3e4e6;">' +
                '<thead><tr>' +
                    ...
                    '<th style="text-align:center; width:10%;">Tgl Bayar Hutang</th>' +
                    '<th style="text-align:center; width:10%;">Jenis Bayar</th>' +
                    '<th style="text-align:center; width:10%;">Nominal</th>' +
                    '<th style="text-align:center; width:10%;">Dibayar Oleh</th>' +
    ...
    function format(d, n){        
        ...
        dthistory.forEach(function(key) {
            ...
            var hdn = (key.JenisTransaksiKas == 'DP PEMBELIAN' || key.KodeTahun != tahunaktif) ? 'hidden' : '';
            $('#isian'+n).append('<tr>' +
    ...
    $(document).ready(function() {
        $("#form-kirimbayar").submit(function(e) {
            e.preventDefault();
            var self = $(this)
        ...
        $("#table-kirimbayar").on("click", ".simpanperrow", function() {
    ...
    function kirimpertrans(kode, kode2) {
        ...
        get_response("<?= base_url('transaksi/transaksi_pembelian/kirimpertransaksi') ?>", data, function(response) {
            if (response.status === false) {
                showSwal('error', 'Peringatan', response.msg);
                return false;
            } else {
                showSwal('success', 'Informasi', response.msg).then(function() {
                    window.location.href = "<?= base_url('transaksi/transaksi_kas/jurnalmanual/') ?>" + btoa("trans_beli") + "/" + btoa(unescape(encodeURIComponent(response.idjurnal))) + "/" + btoa(unescape(encodeURIComponent('<?= $IDTransBeli ?>')));
                    self[0].reset();
                    $table.ajax.reload();
                });
    ...
    function simpanbayar(self, data_post) {
        post_response("<?= base_url('transaksi/transaksi_pembelian/kirimpembayaranproses') ?>", data_post, function(response) {
            if (response.status) {
    ...
    function update(self, data_post) {
        post_response("<?= base_url('transaksi/transaksi_pembelian/editpertransaksi') ?>", data_post, function(response) {
            if (response.status) {
    ...
    /* Tanpa Rupiah */
    var tanpa_rupiah = document.getElementById('TotalEdit');
