<script>
    const $table = $('#table-mutasigdg').DataTable({
        "paging": true,
        "lengthChange": false,
        "searching": false,
        "ordering": true,
        ...
        "ajax": {
            "url": "<?= base_url('transaksi/mutasi_gudang'); ?>",
            "type": "GET",
            "data": function(d) {
    ...
    $(document).ready(function() {
        ...
        $("#table-mutasigdg").on("click", ".btnhapus", function() {
    ...
    function hapus(kode) {
        ...
        get_response("<?= base_url('transaksi/mutasi_gudang/hapus') ?>", data, function(response) {
    ...
    function simpan(self, data_post) {
        post_response("<?= base_url('transaksi/mutasi_gudang/simpan') ?>", data_post, function(response) {
            if (response.status) {
                ...
                showSwal("success", "Informasi", "Berhasil input data").then(function() {
                    ...
                    if (response.action === 'tambah') {
                        window.location.href = "<?= base_url('transaksi/mutasi_gudang/detail/') ?>" + btoa(unescape(encodeURIComponent(response.id)));
                    }
                });
            } else {
                $('#ModalTambah').modal('hide');
    ...
    $("#table-mutasigdg").on("click", ".btnedit", function() {
    ...
    $("#btntambah").on("click", function() {
        ...
        $('#GudangAsal').change(function () {
            var gudangasal = $(this).find('option:selected').attr('value');
            $.ajax({
                url: "<?php echo site_url('user/Lokasi/DataGudangTujuan'); ?>",
                method: "GET",
                    ...
                    if (gudangasal) {
                        ...
                        $('#GudangTujuan').select2({
                            ...
                            ajax: {
                                dataType: 'json',
                                delay: 250,
                                url: '<?php echo base_url('user/Lokasi/DataGudangTujuan'); ?>',
                                data: function(params) {
                                    return {
